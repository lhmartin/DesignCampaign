...
function CdrButton({ chains, activeFile, onNeedSetup }: {
  ...
  const cdrRunning    = useAntpackStore(s => !!activeFile && s.runningPaths.has(activeFile))
  const anyRunning    = useAntpackStore(s => s.runningPaths.size > 0)
  const cdrError      = useAntpackStore(s => activeFile ? s.errorsByPath.get(activeFile) : undefined)
  const annotation    = useAntpackStore(s => activeFile ? s.annotationsByPath.get(activeFile) : undefined)
