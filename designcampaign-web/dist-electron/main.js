...
function registerIpcHandlers(getMainWindow) {
  ipcMain.handle("dialog:openFolder", async () => {
    ...
    return fs$j.statSync(selected).isDirectory() ? selected : path$m.dirname(selected);
  });
  ...
  ipcMain.handle("fs:readFile", async (_event, filePath) => {
    ...
    return fs$j.readFileSync(resolved, "utf-8");
  });
  ipcMain.handle("fs:readFileBinary", async (_event, filePath) => {
    const resolved = path$m.resolve(filePath);
    const buffer = fs$j.readFileSync(resolved);
    return buffer;
  });
  ipcMain.handle("fs:writeFile", async (_event, filePath, data) => {
    const resolved = path$m.resolve(filePath);
    fs$j.mkdirSync(path$m.dirname(resolved), { recursive: true });
    fs$j.writeFileSync(resolved, data, "utf-8");
  });
  ipcMain.handle("fs:listFiles", async (_event, dir, extensions) => {
    const resolved = path$m.resolve(dir);
    if (!fs$j.existsSync(resolved)) return [];
    const files = [];
    const exts = extensions.map((e) => e.toLowerCase());
    function scan(dirPath) {
      const entries = fs$j.readdirSync(dirPath, { withFileTypes: true });
      for (const entry of entries) {
        const fullPath = path$m.join(dirPath, entry.name);
        if (entry.isDirectory()) {
          scan(fullPath);
        } else if (entry.isFile()) {
          const ext = path$m.extname(entry.name).toLowerCase();
          if (exts.includes(ext)) {
            const stat2 = fs$j.statSync(fullPath);
            files.push({
              name: entry.name,
              path: fullPath,
              size: stat2.size,
  ...
  ipcMain.handle("fs:getFileStats", async (_event, filePath) => {
    const resolved = path$m.resolve(filePath);
    const stat2 = fs$j.statSync(resolved);
    return { size: stat2.size, mtime: stat2.mtimeMs };
  });
  ipcMain.handle("fs:watchFolder", async (_event, dir) => {
    const resolved = path$m.resolve(dir);
    const win2 = getMainWindow();
    if (!win2 || !fs$j.existsSync(resolved)) return;
    const watcher = fs$j.watch(resolved, { recursive: true }, (event, filename) => {
      if (filename) {
        win2.webContents.send("folder-changed", { event, path: path$m.join(resolved, filename) });
      }
    });
...
function createWindow() {
  win = new BrowserWindow({
    ...
    webPreferences: {
      preload: path$m.join(__dirname$1, "preload.cjs"),
